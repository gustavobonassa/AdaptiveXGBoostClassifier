...
class AdaptiveXGBoostClassifier2(BaseSKMObject, ClassifierMixin):
    _PUSH_STRATEGY = 'push'
    ...
    def _train_booster(self, X: np.ndarray, y: np.ndarray, last_model_idx: int):
        ...
        for j in range(last_model_idx):
            if self._ensemble[j]:
                self._ensemble[j] = xgb.train(params=self._boosting_params,
                                    dtrain=d_mini_batch_train,
                                    num_boost_round=1,
                                    xgb_model='./models/model_'+str(j)+'.model')
                self._ensemble[j].save_model('./models/model_'+str(j)+'.model')
